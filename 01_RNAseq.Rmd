...
```{r}
dat_dir <- "differential_expression"
...
contrast <- c("treatment", "cApUp", "control")
res_capup <- results(dds, contrast = contrast)
res_lfc_capup <- lfcShrink(dds, contrast = contrast, type = "ashr")
...
contrast <- c("treatment", "cGAMP", "control")
res_cgamp <- results(dds, contrast = contrast)
res_lfc_cgamp <- lfcShrink(dds, contrast = contrast, type = "ashr")
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, sprintf("volcano_plots_padj%s.pdf", padj_thr)),
  width = 10, height = 12
)
gp_pch
dev.off()
...
# go results
go_map <- readRDS(file.path("annotation", "gomap_spis.RDS"))
tgalg <- "classic"
...
plot_dt[, gene_num := Significant]#[gene_num > 100, gene_num := 100]
...
# kegg pathways
ko <- fread(
  file.path("annotation", "Stylophora_KEGGpathway_table"),
  header = FALSE
)
...
# load kegg info
ko <- fread(file.path("annotation", "Stylophora_KEGGKO_table"), header = FALSE)
setnames(ko, c("gene", "ko"))
