...
# label immune genes
label_genes <- gene_ann[grep("NFkB|IRF|STAT\\d+", pfam, ignore.case = TRUE)]
label_genes_immuno_1 <- structure(label_genes$pfam, names = label_genes$gene)

immune_dt <- fread(file.path(ann_dir, "immune.txt"), header = FALSE)
label_genes_immuno_2 <- structure(immune_dt[[2]], names = immune_dt[[1]])
...
label_genes_immuno <- c(
  ...
  label_genes_immuno_1[
    names(label_genes_immuno_1) %in% names(label_genes_immuno_2)
  ], use.names = TRUE
)
...
names(label_genes_v) <- names(label_genes_immuno)
...
res_lfc_capup$label_gene <- label_genes_v[rownames(res_lfc_capup)]
...
gp_capup_vo <- ggplotVolcano(
  ...
  lims_fc = c(-5, 5), lims_sig = c(NA, 35),
...
gp_cgamp_vo <- ggplotVolcano(
  ...
  lims_fc = c(-5, 5), lims_sig = c(NA, 50),
  sign_col = c("#F27333", "#725CA6"), size = 0.9, alpha = 0.8,
...
gp_capup_vo + gp_cgamp_vo
```

```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, sprintf("dds_res_ashr_cApUp_padj%s_2.pdf", padj_thr)),
  width = 5, height = 6, useDingbats = TRUE
)
gp_capup_vo
dev.off()

pdf(
  file.path(fig_dir, sprintf("dds_res_ashr_cGAMP_padj%s_2.pdf", padj_thr)),
  width = 5, height = 6
)
gp_cgamp_vo
dev.off()
...
dt_sign <- rbindlist(sapply(names(res_list), function(x) {
  res_list[[x]][
    padj < 0.05 & abs(log2FoldChange) > lfc_thr &
    (cGLR == TRUE | STING == TRUE)][
...
dt_tfs <- rbindlist(sapply(names(res_list), function(x) {
  res_list[[x]][
    padj < 0.05 & abs(log2FoldChange) > lfc_thr & (TF == TRUE)][
      order(-log2FoldChange)][, 1:8]
}, simplify = FALSE, USE.NAMES = TRUE), idcol = "treatment")
...
# up-regulated
res_up <- res_lfc[padj < padj_thr & log2FoldChange > lfc_thr]
...
# down-regulated
res_down <- res_lfc[padj < padj_thr & log2FoldChange < -1 * lfc_thr]
...
```{r}
up_capup <- res_up[gene %in% res_up[,.N,gene][N<2]$gene][treatment == "cApUp"]
up_capup <- merge.data.table(
  up_capup, gene_ann, by = "gene",
  all.x = TRUE, sort = FALSE
)
up_capup[is.na(og), og := ""][is.na(pfam), pfam := ""][is.na(name), name := ""]
fwrite(
  up_capup, file.path(res_dir, "cApUp_up_only.tsv"),
  sep = "\t"
)
```
...
kegg_lst <- sapply(kegg_res, function(x) {
  dt <- as.data.table(x[["greater"]], keep.rownames = "kegg.id")
...
fwrite(
  kegg_dt,
  file.path(kegg_dir, "kegg.results.tsv"),
  sep = "\t"
)
...
# plot
plot_dt <- kegg_dt[order(q.val)][kegg.id %in% kegg_sig[q.val < 0.01]$kegg.id]
plot_dt[, label := sprintf("%s | %s", kegg.id, kegg.lab)]
plot_dt[, kegg_id := factor(
  kegg.id,
  levels = unique(
    plot_dt[group == "cGAMP"][order(stat.mean)][["kegg.id"]]
  )
)]
