...
gp_capup_ma <- ggplotMA(
  res_lfc_capup, padj_thr = padj_thr, lfc_thr = 1,
  lims_fc = c(-5, 5), lims_mean = c(NA, NA), trans_mean = "log10",
  title = sprintf("%s vs %s", contrast_capup[2], contrast_capup[3])
)
...
kegg_dt <- fread(file.path(
  kegg_dir, sprintf("kegg.results.padj%s.tsv", padj_thr)
))
...
# plot
plot_dt <- kegg_sig[order(q.val)][1:20][q.val < 0.01]
...
gp <- ggplot(plot_dt, aes(x, kegg_id, size = change, fill = q.val)) +
  geom_point(shape = 21) +
  labs(
    x = "", y = "KEGG pathway",
    size = "expression\nchange",
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, sprintf("kegg_plot_padj%s.pdf", padj_thr)),
  height = 3,
  width = 8.5,
  useDingbats = TRUE
)
...
fwrite(
  plot_dt,
  file.path(res_dir, sprintf("kegg_plot_padj%s.tsv", padj_thr)),
  sep = "\t"
)
```
