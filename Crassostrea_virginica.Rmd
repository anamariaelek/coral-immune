...
dt_sign <- rbindlist(sapply(names(res_list), function(x) {
  res_list[[x]][padj < 0.05][order(-log2FoldChange)][, 1:8]
...
gp_pch <- gp_cgamp_vo / gp_cgamp_vo_adj
gp_pch
```
...
# gage
kegg_res <- sapply(c("up", "down"), function(i) {
  kegg_gage <- gage(
    exprs = structure(
