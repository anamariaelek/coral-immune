...
gene_ann <- merge.data.table(gene_ann_og, spis_ann, by = "gene_spis", all.x = TRUE)
...
label_genes <- rbindlist(list(
  gene_ann[grep("NFkB|IRF|STAT\\d+", pfam, ignore.case = TRUE)][
...
label_genes_v <- structure(label_genes$name, names = label_genes$gene)
