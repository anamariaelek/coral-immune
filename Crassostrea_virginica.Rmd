...
gp_cgamp_vo <- ggplotVolcano(
  res_cgamp, padj_thr = NULL, pval_thr = padj_thr, lfc_thr = 1,
  lims_fc = c(-5, 5), lims_sig = c(NA, 6),
  title = sprintf("%s vs %s", contrast_cgamp[2], contrast_cgamp[3]),
  ylab = "-log10 p value"
)
gp_cgamp_vo_adj <- ggplotVolcano(
  res_cgamp, padj_thr = padj_thr, lfc_thr = 1,
  lims_fc = c(-5, 5), lims_sig = c(NA, 6),
  title = sprintf("%s vs %s", contrast_cgamp[2], contrast_cgamp[3])
)
...
```{r include=FALSE, eval=TRUE}
pdf(
  file.path(fig_dir, sprintf("dds_res_cGAMP_padj%s.pdf", padj_thr)),
  width = 5, height = 12
)
gp_cgamp_vo / gp_cgamp_vo_adj
dev.off()
pdf(
...
```{r}
res_lfc <- as.data.table(res_lfc_capup, keep.rownames = "gene")
...
# plot
cats <- go_dt[,
  .SD[group == "up"][order(pval_adj)][1:20][pval_adj < 0.01],
  .(ontology)
]$GO.ID
